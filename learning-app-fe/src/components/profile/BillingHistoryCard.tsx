...
export default function BillingHistoryCard({ isDark: dark }: BillingHistoryCardProps) {
    ...
    const getStatusColor = (expiredAt: string) => {
        const now = new Date();
        const expiry = new Date(expiredAt);
        return expiry > now ? "text-green-500 bg-green-500/10" : "text-gray-400 bg-gray-400/10";
    };
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    <div className="flex items-center gap-4 flex-1">
                                        ...
                                        <div className="min-w-0">
                                            <div className="flex items-center gap-2">
                                                <h4 className={`font-bold text-sm truncate ${dark ? "text-white" : "text-gray-900"}`}>{order.packageName}</h4>
                                                <span className={`px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-tighter ${order.planType === 'VIP' ? 'bg-indigo-500 text-white' : 'bg-gray-200 text-gray-600'}`}>
                                                    {order.planType}
                                                </span>
                                            </div>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="flex items-center justify-end gap-1.5 mt-1">
                                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${getStatusColor(order.expiredAt)}`}>
                                                {new Date(order.expiredAt) > new Date() ? "Đang hiệu lực" : "Hết hạn"}
                                            </span>
